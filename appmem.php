...
<div class="container-fluid">
    <div class="row">
     ...
     <?php echo '<img src="file/'.Session::get('picture').'" style="margin-top:-14%; width:90px; height:90px; border-radius:45px 45px 45px 45px; margin-left:60%"/>'; ?>
         ...
         <ul>
             ...
             <li><a href="comment.php">Contact Message</a></li>
    ...
    <div class="plateuser">
        ...
        <h3 style="padding-top:2rem; text-shadow: .1rem .1rem .5rem black;">Member Application Page</h3>
        ...
        <?php 
            ...
            <?php 
                if (isset($_GET['dis'])) {
                    $disid = (int)$_GET['dis'];
                    $disUser= $usr->DisableMem($disid);
                }
                if (isset($_GET['ena'])) {
                    $ensid = (int)$_GET['ena'];
                    $enUser= $usr->EnableMem($ensid);
                }
                if (isset($_GET['del'])) {
                    $delsid = (int)$_GET['del'];
                    $delUser= $usr->DeleteMem($delsid);
                }
            ?>
            ...
            ?>
                        ...
                        <tr>
                            ...
                            <?php if ($result['status']=='0') {?>
                                <a onclick = "return confirm('Are you sure you want to Disable?')" href="?dis=<?php echo $result['id']; ?>">Disapprove </a>
                            <?php  } else {?>
                                <a onclick = "return confirm('Are you sure you want to Enable?')" href="?ena=<?php echo $result['id']; ?>">Approves </a>
                          
                            <?php }?>
                              || <a onclick = "return confirm('Are you sure you want to remove?')" href="?del=<?php echo $result['id']; ?>">Remove </a>
                       </td>
                        </tr>
                            <?php }} ?>        
                    </table>
